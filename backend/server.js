...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/discover') { const user = requireUser(request, response, origin); if (!user) return; const body = await readBody(request); const provider = sanitizeText(body.provider || 'demo', 30); const query = sanitizeText(body.query, 200); const targets = await discoverTargets(provider, { query }); const pipeline = getOverview(user.id).pipeline; replaceTargets(user.id, pipeline.id, targets); recordActivity(user.id, 'pipeline_run'); targets.filter(target => target.match >= 85).forEach(() => recordActivity(user.id, 'qualified_lead')); return json(response, 200, { targets }, origin); }
